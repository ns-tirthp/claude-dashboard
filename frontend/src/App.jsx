...
function App() {
  ...
  const branchesForSelectedProject =
    selectedProject === "all"
      ? []
      : filterOptions.projects.find((p) => p.name === selectedProject)
          ?.branches || [];
  ...
  const TIME_SOURCE_LABEL = {
    ...
    estimated:
      "Estimated from message timestamps (SDK / IDE session). Each turn is capped at 5 minutes to limit idle-time inflation.",
    mixed:
      "Some sessions measured (CLI), others estimated from timestamps (SDK / IDE). Total combines both.",
    ...
    return (
      <div className="min-h-screen bg-[#EEEEE8] flex items-center justify-center">
        <div className="text-center">
          <Spin size="large" />
          <p
            className="mt-4 font-mono uppercase tracking-wider text-sm"
            style={{ color: INK, letterSpacing: "0.08em" }}
          >
            Loading statistics...
          </p>
        </div>
  ...
  const modelChartData = Object.entries(stats.modelUsage).map(
    ([name, count]) => ({
  ...
  const entrypointChartData = Object.entries(stats.entrypointUsage || {}).map(
  ...
  const avgToolCallsPerConversation =
    stats.totalConversations > 0
      ? Math.round(stats.totalToolCalls / stats.totalConversations)
      : 0;

  const avgTimePerConversation =
    stats.totalConversations > 0
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      render: (val) => (
        <span style={{ fontWeight: 700 }}>{formatNumber(val)}</span>
      ),
    },
    {
      title: "Time Spent",
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ fontSize: "13px", color: INK, fontWeight: 700 }}>
          {formatNumber(record.tokens.input)} /{" "}
          {formatNumber(record.tokens.output)}
        </div>
      ),
    },
    {
      ...
      render: (_, record) => (
        <div style={{ fontSize: "13px", color: INK, fontWeight: 700 }}>
          {formatNumber(record.tokens.cacheCreation)} /{" "}
          {formatNumber(record.tokens.cacheRead)}
        </div>
      ),
    },
    {
      ...
      render: (_, record) => {
        ...
        const topBranch = Object.entries(record.branches || {}).sort(
          (a, b) => b[1] - a[1],
        )[0];
        return (
          <div style={{ fontSize: "13px", color: INK, fontWeight: 700 }}>
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ fontSize: "13px", color: INK, fontWeight: 700 }}>
          {record.fileOperations?.edits || 0} /{" "}
          {record.fileOperations?.reads || 0} /{" "}
          {record.fileOperations?.writes || 0}
        </div>
      ),
    },
    {
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="dashboard-header">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl mb-1">Claude Usage Dashboard</h1>
              <p
                className="text-sm"
                  ...
                  <div className="filter-bar">
                    ...
                    <div className="filter-bar-control">
                      ...
                      <Select
                        ...
                        disabled={
                          selectedProject === "all" ||
                          branchesForSelectedProject.length === 0
                        }
                        style={{ width: 220 }}
                        options={[
                          { value: "all", label: "All Branches" },
                          ...branchesForSelectedProject.map((b) => ({
                            value: b,
                            label: b,
                          })),
                        ]}
                      ...
                      <Card className="stat-card-gradient-amber">
                        <Statistic
                          title={
                            <span>
                              Total Time Spent
                              {(stats.timeSource === "estimated" ||
                                stats.timeSource === "mixed") && (
                                <AntTooltip
                                  title={TIME_SOURCE_LABEL[stats.timeSource]}
                                >
                                  <Tag
                                    style={{
                                      marginLeft: 8,
                  ...
                  <Row gutter={[24, 24]} className="mb-8">
                    <Col xs={24} md={8}>
                      <Card>
                        ...
                        <Progress
                          percent={Math.min(
                            (avgToolCallsPerConversation / 50) * 100,
                            100,
                          )}
                          strokeColor={INK}
                          trailColor={RULE}
                    ...
                    <Col xs={24} md={8}>
                      <Card>
                        <Statistic
                          ...
                          prefix={
                            <ThunderboltOutlined style={{ color: INK }} />
                          }
                          valueStyle={{ color: INK }}
                          suffix="%"
                        />
        ...
        <div className="dashboard-footer">
          ...
          <p style={{ margin: 0 }}>
            ...
            Data sourced from ~/.claude/projects/ · Last updated:{" "}
